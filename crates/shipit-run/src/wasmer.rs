//! Port of `src/shipit/runners/wasmer.py` (WasmerRunner).
//!
...
pub const SHIPIT_CONFIG_ANNOTATION: &str = "shipitcli.com/config";
...
impl WasmerRunner {
    pub fn new(
        ...
        token: Option<String>,
        bin: Option<String>,
        shipit_dir: Option<PathBuf>,
    ) -> Self {
        let shipit_dir = shipit_dir.unwrap_or_else(|| src_dir.join(".shipit"));
        let wasmer_dir_path = shipit_dir.join("wasmer");
        Self {
            build_backend,
            src_dir,
            shipit_dir,
...
impl Runner for WasmerRunner {
    ...
    fn prepare(&mut self, _env: &IndexMap<String, String>, _prepare: &[RunStep]) -> Result<()> {
        let prepare_dir = self.wasmer_dir_path.join("prepare");
        let mut volume_mappings = load_volume_mappings(&self.shipit_dir)?;
...
mod tests {
    ...
    use super::*;
    use shipit_plan::{Mount, Volume};
    use shipit_providers::node::NodeConfig;
    use shipit_providers::python::{PythonConfig, PythonFramework};
    ...
    fn test_config_annotation_matches_python_byte_for_byte() {
        // Golden captured from the Python implementation (PythonConfig with
    ...
    fn test_wasmer_app_yaml_updates_existing_volume_with_same_mount() {
        ...
        serve.volumes = Some(vec![
            ...
            Volume {
                ...
                path: tmp.path().join(".shipit").join("volumes").join("uploads"),
