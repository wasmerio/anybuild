//! Port of `src/shipit/runners/local.py` (LocalRunner).
//!
...
use shipit_build::local::ui::{console_print, print_panel};
...
impl LocalRunner {
    pub fn new(
        ...
        let runner_path = shipit_dir.join("runner").join("local");
    ...
    fn build_serve(&mut self, serve: &Serve) -> Result<()> {
        ...
        console_print("Serve Commands:");
        for (command, command_body) in &serve.commands {
            console_print(&format!("* {command}"));
