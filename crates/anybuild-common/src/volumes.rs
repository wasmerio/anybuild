...
pub fn volumes_dir(anybuild_dir: &Path) -> PathBuf {
    anybuild_dir.join("volumes")
}

pub fn volume_mappings_path(anybuild_dir: &Path) -> PathBuf {
    volumes_dir(anybuild_dir).join("mappings.json")
}

/// Load persisted volume name to guest-path mappings.
///
/// Missing mapping files represent a project without volumes. Non-string
/// values retain the legacy `str(value)`-like JSON form.
pub fn load_volume_mappings(anybuild_dir: &Path) -> Result<IndexMap<String, String>> {
    let mappings_path = volume_mappings_path(anybuild_dir);
