...
use shipit_e2e::BuildMode::{Local, Wasmer, WasmerAndDocker};
...
macro_rules! e2e_tests {
    ($($name:ident => ($id:literal, $mode:expr);)*) => {
        $(
            ...
            fn $name() {
                if let Err(err) = shipit_e2e::run_case($id, $mode) {
                    panic!("{err:#}");
                }
            }
        )*
        ...
        fn e2e_test_list_matches_case_table() {
            let generated: Vec<(&str, &str, shipit_e2e::BuildMode)> = vec![
                $( (stringify!($name), $id, $mode) ),*
            ];
            shipit_e2e::verify_test_list(&generated);
        }
    };
}
