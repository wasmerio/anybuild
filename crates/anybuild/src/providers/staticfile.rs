//! Static-file provider implementation.
//!
...
/// documents Anybuild reads (Staticfile roots, Jekyll destinations). Only
...
) -> Result<StaticFileConfig> {
    let mut config = load_static_config(path, base, operation)?;
    ...
    if staticfile_path.exists() {
        if let Ok(contents) = std::fs::read_to_string(&staticfile_path) {
            ...
            if !parsed.is_empty() {
                ...
                let mut config = StaticFileConfig::from_env(base, operation)?;
...
fn is_unbuilt_node_static_site(
    ...
    let Some(package_json) = crate::providers::node::parse_package_json(path) else {
        return false;
    };
    let scripts = crate::providers::node::package_scripts(Some(&package_json));
    if scripts
        .get("start")
    ...
    crate::providers::node::infer_start_command(path, Some(&package_json), false, operation)
        .ok()
        .flatten()
        .is_none()
}
