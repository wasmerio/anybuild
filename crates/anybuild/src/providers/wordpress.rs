...
pub fn load_config(
...
) -> Result<WordPressConfig> {
    let php_config = php::load_config(path, base, operation)?;
    ...
    let mut config = WordPressConfig {
        php: php_config,
        wp_version: env_str(operation, "wp_version"),
        wp_locale: env_str(operation, "wp_locale"),
        wp_cli_version: env_str(operation, "wp_cli_version"),
        wp_extension_kind: env_enum(operation, "wp_extension_kind", "plugin or theme", |value| {
            matches!(value, "plugin" | "theme").then(|| value.to_owned())
        })?,
        wp_extension_slug: env_str(operation, "wp_extension_slug"),
        wp_extension_activate_target: env_str(operation, "wp_extension_activate_target"),
    };
...
mod tests {
    ...
    //! (`crates/anybuild/src/tests/starlark_snapshots.rs`); the config fields that
