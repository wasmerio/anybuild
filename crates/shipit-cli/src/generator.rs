...
#[cfg(test)]
mod tests {
    use super::shipit_version;

    /// The committed example goldens must carry the crate version in
    /// their headers — a stale golden regeneration shows up here.
    #[test]
    fn goldens_carry_the_crate_version() {
        let golden = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../examples/staticfile/Shipit");
        let text = std::fs::read_to_string(&golden).unwrap();
        assert!(
            text.contains(&format!("Generated by Shipit v{}", shipit_version())),
            "examples/*/Shipit headers are stale; regenerate the goldens"
        );
    }
}
