//! `evaluate_shipit`: evaluate a Shipit file into a Serve plan.
//!
//! Port of `evaluator.py::evaluate_shipit`. The provider config arrives as
...
use crate::ctx::{shipit_builtins, Ctx};
use crate::loader::{ModuleGraph, StdlibSource};
...
fn globals(port: &str, config: Option<&serde_json::Map<String, serde_json::Value>>) -> Globals {
    let mut builder = GlobalsBuilder::extended_by(LIBRARY_EXTENSIONS).with(shipit_builtins);
...
pub fn evaluate_shipit(options: EvaluateOptions) -> Result<Serve> {
    let EvaluateOptions {
        shipit_file,
        project_root,
        config,
    ...
    let source = std::fs::read_to_string(&shipit_file)
        .with_context(|| format!("reading {}", shipit_file.display()))?;
    let workspace_root = match project_root {
        Some(root) => root,
        None => shipit_file
            .canonicalize()
            .unwrap_or_else(|_| shipit_file.clone())
            .parent()
            .ok_or_else(|| anyhow!("Shipit file has no parent directory"))?
            .to_path_buf(),
    };
    ...
    if serves.is_empty() {
        bail!("No serve definition found in {}", shipit_file.display());
    }
    if serves.len() > 1 {
    ...
    if !had_loads {
        // Legacy fully-inlined Shipit files predate the Starlark stdlib's
