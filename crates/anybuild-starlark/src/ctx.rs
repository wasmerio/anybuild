//! The evaluation context and the builtins exposed to Anybuild files.
//!
...
use anybuild_plan::layout::MountLayout;
use anybuild_plan::{
...
fn ctx<'a>(eval: &Evaluator<'_, 'a, '_>) -> Result<&'a Ctx> {
    eval.extra
        .ok_or_else(|| anyhow!("anybuild evaluation context missing"))?
        .downcast_ref::<Ctx>()
        .ok_or_else(|| anyhow!("anybuild evaluation context has the wrong type"))
}
...
pub fn anybuild_builtins(builder: &mut GlobalsBuilder) {
    ...
    /// The stdlib's `serve()` assembler (//anybuild:serve.bzl) shadows the
