//! Port of `src/shipit/builders/docker.py` (DockerBuildBackend).
//!
...
pub struct DockerBuildBackend {
    ...
    pub shipit_docker_path: PathBuf,
...
impl DockerBuildBackend {
    pub fn new(
        ...
        let docker_path = shipit_dir.join("docker");
        ...
        Ok(Self {
            ...
            docker_path,
            shipit_docker_path: PathBuf::from("/shipit"),
                ...
                Step::Copy(step) => {
                    if step.is_download() {
                        ...
                        if asset_path.is_file() {
                            ...
                            bail!(
                                "Asset {} is a directory, shipit doesn't currently support coppying assets directories inside Docker",
                                step.source
                            );
                        } else {
...
impl BuildBackend for DockerBuildBackend {
    ...
    fn build(
        ...
        std::fs::write(&self.docker_ignore_path, "\n.shipit\nShipit\n")?;
...
mod tests {
    ...
    fn test_mount_paths() {
        ...
        assert_eq!(
            ...
            backend.shipit_dir.join("volumes").join("db")
        );
        assert!(!backend.is_depot());
    }
    ...
    fn test_dockerfile_asset_errors() {
        ...
        assert_eq!(
            ...
            "Asset dir is a directory, shipit doesn't currently support coppying assets directories inside Docker"
        );
    }
