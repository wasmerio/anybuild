...
mod tests {
    ...
    fn goldens_carry_the_crate_version() {
        let golden = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../examples/staticfile/Anybuild");
        let text = std::fs::read_to_string(&golden).unwrap();
        assert!(
            text.contains(&format!("Generated by Anybuild v{}", anybuild_version())),
            "examples/*/Anybuild headers are stale; regenerate the goldens"
        );
    }
}
