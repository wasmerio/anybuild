//! `anybuild run` (port of cli.py's run command).
//!
...
pub fn run_serve_commands(
    ...
    anybuild_dir: Option<&std::path::Path>,
) -> Result<()> {
    let volume_mappings = merge_volume_mappings(&[
        load_volume_mappings(path, anybuild_dir)?,
...
mod tests {
    ...
    fn run_loads_volume_mappings_from_json() {
        ...
        let mappings_dir = anybuild_dir.join("volumes");
