...
fn run_case_inner(
    ...
    if case.download.is_some() || !case.commands.is_empty() {
        ...
        let run_cmd = anybuild_run_command(
            repo_root,
            project_path,
            build_mode,
...
fn serve_time_checks(
    ...
    if case.expected_memory_limit.is_some() || case.expect_no_memory_limit {
        let app_yaml_path = project_path
            .join(".anybuild")
            .join("wasmer")
            .join("app.yaml");
...
// ---------------------------------------------------------------------------
// anybuild invocation builders (ports of `_anybuild_*_command`)
// ---------------------------------------------------------------------------

/// The anybuild invocation under test. `ANYBUILD_BIN` (split on whitespace)
/// overrides the default workspace debug binary.
fn anybuild_base_command(repo_root: &Path) -> Result<Vec<String>> {
    if let Ok(bin) = std::env::var("ANYBUILD_BIN") {
        if !bin.trim().is_empty() {
            return Ok(bin.split_whitespace().map(str::to_string).collect());
        }
    }
    let default = repo_root.join("target").join("debug").join("anybuild");
    ensure!(
        default.is_file(),
        "anybuild binary not found at {}; build it with \
         `cargo build -p anybuild-cli` or point ANYBUILD_BIN at a binary",
        default.display()
    );
    Ok(vec![default.to_string_lossy().into_owned()])
}

fn anybuild_auto_command(
    repo_root: &Path,
    project_path: &Path,
    build_mode: BuildMode,
    port: u16,
...
fn create_mysql_database(envs: &[(String, String)], repo_root: &Path) -> Result<String> {
    let name = format!("anybuild_e2e_{:016x}{:016x}", rand_u64(), rand_u64());
    let sql = format!(
...
fn create_wp_content_volume(project_path: &Path) -> Result<PathBuf> {
    let host_dir = PathBuf::from(format!("/tmp/anybuild-e2e-wp-content-{:016x}", rand_u64()));
